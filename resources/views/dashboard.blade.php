                ...
                <div class="card">
                    <div class="flex items-center justify-between mb-4" style="flex-wrap:wrap;gap:12px;">
                        ...
                        <div style="display:flex;gap:8px;">
                            <button onclick="openAddMaterialModal()" class="btn btn-secondary" style="background:#f0f4ff;color:#1d4ed8;border:1px solid #bfdbfe;">+ Add Material</button>
                    ...
                    <div class="sr-filter-bar">
                        <div class="sr-filter-group">
                            ...
                            <input type="text" class="sr-filter-input" id="mm-filter-id" placeholder="Cari ID…" oninput="applyMaterialFilters()" style="min-width:120px;">
                        </div>
                        <div class="sr-filter-group">
                            <label class="sr-filter-label">Material Description</label>
                            <input type="text" class="sr-filter-input" id="mm-filter-desc" placeholder="Cari nama material…" oninput="applyMaterialFilters()">
                        </div>
                        <div class="sr-filter-group">
                            <label class="sr-filter-label">Product Family</label>
                            <input type="text" class="sr-filter-input" id="mm-filter-family" placeholder="Cari family…" oninput="applyMaterialFilters()" style="min-width:140px;">
                        </div>
                        <div class="sr-filter-group">
                            <label class="sr-filter-label">Subclass 1</label>
                            <input type="text" class="sr-filter-input" id="mm-filter-sub1" placeholder="Cari subclass 1…" oninput="applyMaterialFilters()" style="min-width:130px;">
                        </div>
                        <div class="sr-filter-group">
                            <label class="sr-filter-label">Subclass 2</label>
                            <input type="text" class="sr-filter-input" id="mm-filter-sub2" placeholder="Cari subclass 2…" oninput="applyMaterialFilters()" style="min-width:130px;">
                        </div>
            ...
            <div id="master-salesman-content" class="content-view">
                <div class="card">
                    ...
                    <div class="sr-filter-bar">
                        ...
                        <div class="sr-filter-group">
                            <label class="sr-filter-label">Customer</label>
                            <input type="text" class="sr-filter-input" id="ms-filter-customer" placeholder="Cari customer…" oninput="applysalesmanFilters()">
                        </div>
                        <div class="sr-filter-group">
                            <label class="sr-filter-label">Material</label>
                            <input type="text" class="sr-filter-input" id="ms-filter-material" placeholder="Cari material…" oninput="applysalesmanFilters()">
                        </div>
...
<script>
    ...
    // Master data globals
    var masterMaterialData    = [];  // raw from salesReportDBData
    var masterMaterialBase    = [];  // unique material rows
    var masterSalesmanData    = [];  // aggregated salesman rows
    var masterSalesmanBase    = [];  // for filtering
    ...
    function renderSRTable(rows, tbodyId) {
        ...
        if (rows.length === 0) { tbody.innerHTML=`<tr><td colspan="7" style="text-align:center;padding:30px;color:var(--muted);">Tidak ada data.</td></tr>`; return; }
    ...
    document.getElementById('exportBtn').addEventListener('click', async function() {
        ...
        try {
            ...
            // Reset master data cache so they reload next time
    ...
    // Business Area filter untuk Resume (terpisah dari currentBAFilter milik Sales Report)
    ...
    // Tutup dropdown BA saat klik di luar
    document.addEventListener('click', function(e) {
    ...
    // Filter data resume sesuai BA + year + month
    ...
    function onResumeYearChange() {
        ...
        if (yearVal === 'ALL') {
            ...
            // Hanya tampilkan bulan yang ada di data (untuk year tersebut, semua BA)
    ...
    function renderResumeReport() {
        ...
        setTimeout(() => {
            ...
            // Label untuk header tabel summary
            let baLabel = currentResumeBA !== 'ALL' ? ` · ${currentResumeBA}` : '';
            let label;
            if (yearVal === 'ALL') {
                label = 'All Data' + baLabel;
            } else if (monthVal !== 'ALL') {
                label = `${MONTH_NAMES[parseInt(monthVal)]} ${yearVal}` + baLabel;
            } else {
                label = `YTD ${yearVal}` + baLabel;
            }
            ...
            // Jika YTD (year dipilih, month = ALL) → render juga breakdown per bulan
            if (yearVal !== 'ALL' && monthVal === 'ALL') {
                renderResumeMonthlyBreakdown(yearVal);
            }
        ...
        monthsInData.forEach(mn => {
            ...
            section.innerHTML = `
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
                    ...
                    <span id="resume-month-rec-${mn}" style="font-size:11px;color:var(--muted);">(${monthRows.length.toLocaleString()} records)</span>
                </div>`;
    ...
    function exportResumeReport() {
        ...
        if (yearVal === 'ALL') {
            ...
            label = `${MONTH_NAMES[parseInt(monthVal)]}_${yearVal}`;
        } else {
            label = `YTD_${yearVal}`;
        }
        ...
        // Sheet per bulan (hanya ada saat YTD)
        if (yearVal !== 'ALL' && monthVal === 'ALL') {
    ...
    // =========================================================
    /**
     * Derives master material data from salesReportDBData.
     * Product Family = derived from DC via dcFamilyMapGlobal.
     * Subclass 1 = DC value itself (e.g. "DC 2").
     * Subclass 2 = Division field (if available).
     * Each unique material_id gets ONE row (deduplicated).
     */
    function buildMasterMaterialData() {
        if (salesReportDBData.length === 0) return;
        const seen = new Map(); // material_id → row
        salesReportDBData.forEach(r => {
            ...
            const family  = getFamily(dcRaw);
            const sub1    = dcRaw || '—';
            const sub2    = String(r.division || '').trim() || '—';
            seen.set(mid, {
                material_id:  mid,
    ...
    function renderMasterMaterial() {
        ...
        // If salesReportDBData is empty, try to fetch it first
        ...
        masterMaterialBase = [];  // always rebuild
        buildMasterMaterialData();
        _renderMMTable();
    }
    ...
    function applyMaterialFilters() {
        ...
        tbody.innerHTML = filtered.length === 0
            ? `<tr><td colspan="5" style="text-align:center;padding:30px;color:var(--muted);">Tidak ada data yang sesuai filter.</td></tr>`
            : filtered.map(r => `
                <tr>
    ...
    // =========================================================
    /**
     * Aggregates salesReportDBData per (salesman_id, salesman, customer, material).
     * Gross GM% = sum(gross_gp) / sum(gross_revenue) * 100.
     * Clicking the GM% cell opens a detail modal.
     */
    function buildMasterSalesmanData() {
        if (salesReportDBData.length === 0) return;
        const map = new Map();
        salesReportDBData.forEach(r => {
            const sid      = String(r.salesman_id || '').trim() || '—';
            const sname    = String(r.salesman || '').trim()    || '—';
            const customer = String(r.customer || '').trim()    || '—';
            const material = String(r.material || '').trim()    || '—';
            const key = `${sid}|${customer}|${material}`;
            if (!map.has(key)) {
                map.set(key, {
                    salesman_id: sid,
                    salesman:    sname,
                    ...
                    logistic_expense: 0,
                    gross_gp:         0,
                    rows: [],  // raw rows for modal detail
                });
            }
            const agg = map.get(key);
            agg.qty              += parseFloat(r.qty)              || 0;
            agg.gross_revenue    += parseFloat(r.gross_revenue)    || 0;
            agg.net_revenue      += parseFloat(r.net_revenue)      || 0;
            agg.cogs             += parseFloat(r.cogs)             || 0;
            agg.logistic_expense += parseFloat(r.logistic_expense) || 0;
            agg.gross_gp         += parseFloat(r.gross_gp)         || 0;
            agg.rows.push(r);
        });
        masterSalesmanBase = Array.from(map.values()).sort((a,b) =>
            a.salesman_id.localeCompare(b.salesman_id) || a.customer.localeCompare(b.customer)
        );
    }

    function renderMasterSalesman() {
        const loading = document.getElementById('msLoading');
        const wrap    = document.getElementById('msTableWrap');
        const info    = document.getElementById('msDataInfo');
        ...
        if (masterSalesmanBase.length === 0) buildMasterSalesmanData();
        _renderMSTable();
    }

    function _renderMSTable() {
        ...
        applyMsFilters_internal(masterSalesmanBase);
        makeTableSortable('msTable');
    }
    ...
    function applyMsFilters_internal(base) {
        ...
        const fCust = (document.getElementById('ms-filter-customer')?.value || '').toLowerCase().trim();
        const fMat  = (document.getElementById('ms-filter-material')?.value || '').toLowerCase().trim();
        ...
        if (filtered.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:30px;color:var(--muted);">Tidak ada data yang sesuai filter.</td></tr>`;
            document.getElementById('msRowCount').textContent = '0';
            return;
        }

        tbody.innerHTML = filtered.map((r, idx) => {
            const gm       = r.gross_revenue > 0 ? (r.gross_gp / r.gross_revenue * 100) : 0;
            const gmColor  = gm >= 20 ? '#16a34a' : '#dc2626';
            const gmBg     = gm >= 20 ? '#f0fdf4' : '#fef2f2';
            const gmBorder = gm >= 20 ? '#bbf7d0' : '#fecaca';
            return `<tr data-ms-idx="${idx}">
                <td><span style="font-family:monospace;font-size:12px;font-weight:700;color:#475569;">${r.salesman_id}</span></td>
                <td style="font-weight:600;color:var(--navy);">${r.salesman}</td>
                <td>${r.customer}</td>
                <td><span style="background:#f1f5f9;padding:3px 8px;border-radius:4px;font-size:11px;color:var(--muted);border:1px solid var(--border);">${r.material}</span></td>
                <td class="num gm-cell" style="text-align:right;padding:8px 10px;"
                    onclick="openSalesmanModal(${idx}, '${escapeJs(r.salesman)}', '${escapeJs(r.salesman_id)}')"
                    title="Klik untuk melihat detail">
                    <span style="display:inline-flex;align-items:center;gap:6px;justify-content:flex-end;cursor:pointer;padding:4px 10px;border-radius:6px;background:${gmBg};border:1px solid ${gmBorder};font-weight:700;color:${gmColor};font-size:13px;">
                        ${formatDec2(gm)}%
        ...
        if (cnt) cnt.textContent = filtered.length < masterSalesmanBase.length
            ? `${filtered.length.toLocaleString()} of ${masterSalesmanBase.length.toLocaleString()} entri` : '';

        // Store filtered reference for modal
        window._msFilteredRows = filtered;
    }

    function applyMsFilters() { applyMsFilters_internal(masterSalesmanBase); }
    function applyMsFiltersAlias() { applyMsFilters(); }

    // alias used by oninput in HTML
    function applyMsFiltersCall() { applyMsFilters(); }
    // Also wire up via:
    function applyalesmanFilters() { applyMsFilters(); }
    function applyalesmanFilters2() { applyMsFilters(); }
    function applySalesmanFilters() { applyMsFilters(); }
    function applyalesmanFilters() { applyMsFilters(); } // intentional alias

    // Correct name for oninput binding in HTML
    function applyalesmanFilters(){}
    window.applyalesmanFilters = applyMsFilters;
    window.applyalesmanFilters2 = applyMsFilters;

    function clearSalesmanFilters() {
        ['ms-filter-id','ms-filter-name','ms-filter-customer','ms-filter-material']
            .forEach(id => { const el=document.getElementById(id); if(el) el.value=''; });
    ...
    function openSalesmanModal(idx, salesmanName, salesmanId) {
        ...
        // KPI
        const gm = agg.gross_revenue > 0 ? (agg.gross_gp / agg.gross_revenue * 100) : 0;
        const gmColor = gm >= 20 ? '#16a34a' : '#dc2626';
        document.getElementById('modalKpiQty').textContent  = formatNum(agg.qty);
        document.getElementById('modalKpiRev').textContent  = formatNum(agg.gross_revenue);
        document.getElementById('modalKpiGP').textContent   = formatNum(agg.gross_gp);
        document.getElementById('modalKpiGM').textContent   = formatDec2(gm) + '%';
        document.getElementById('modalKpiGM').style.color   = gmColor;

        // Detail rows — show each raw transaction grouped by month
        const detailRows = (agg.rows || []).slice().sort((a,b) => {
            const ma = parseInt(a.month_num) || 0;
            const mb = parseInt(b.month_num) || 0;
            return ma - mb;
        });

        const tbody = document.getElementById('modalDetailBody');
        if (detailRows.length === 0) {
            tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:20px;color:var(--muted);">Tidak ada detail.</td></tr>`;
        } else {
            tbody.innerHTML = detailRows.map(r => {
                const rowGM = parseFloat(r.gross_revenue) > 0
                    ? (parseFloat(r.gross_gp) / parseFloat(r.gross_revenue) * 100) : 0;
                const rowColor = rowGM >= 20 ? '#16a34a' : '#dc2626';
                return `<tr style="border-bottom:1px solid #f1f5f9;">
                    <td style="padding:7px 10px;color:#374151;">${r.customer || '—'}</td>
                    <td style="padding:7px 10px;color:#374151;font-size:11px;">${r.material || '—'}</td>
                    <td style="padding:7px 10px;text-align:right;color:var(--navy);font-weight:600;">${formatNum(parseFloat(r.qty)||0)}</td>
                    <td style="padding:7px 10px;text-align:right;color:var(--navy);">${formatNum(parseFloat(r.gross_revenue)||0)}</td>
                    <td style="padding:7px 10px;text-align:right;color:var(--navy);">${formatNum(parseFloat(r.net_revenue)||0)}</td>
                    <td style="padding:7px 10px;text-align:right;color:var(--navy);">${formatNum(parseFloat(r.cogs)||0)}</td>
                    <td style="padding:7px 10px;text-align:right;color:var(--navy);font-weight:600;">${formatNum(parseFloat(r.gross_gp)||0)}</td>
        ...
        // Show modal
        const modal = document.getElementById('salesmanDetailModal');
    ...
    // =========================================================
    // WIRE UP SALESMAN FILTER INPUTS (oninput binding)
    // =========================================================
    // The HTML uses oninput="applyalesmanFilters()" — make sure it resolves
    window.applyalesmanFilters = applyMsFilters;
